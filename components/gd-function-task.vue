    ...
    <div class="gd-function-task">
      ...
      <div v-if="kind === 'condition'" class="gd-function-task-body">
        <div class="gd-function-task-section">
          <span class="gd-function-task-section-name gd-headline-6">Condition</span>
          <gd-function-task-condition :master="true" :condition="condition" :sensor="sensor" :relay="relay" />
        </div>
        <div class="gd-function-task-section">
          <span class="gd-function-task-section-name gd-headline-6">
            True action
          </span>
          <gd-input-button v-if="conditionTrueTask.length === 0" label="Add task" type="background"
            @clicked="conditionTrueTaskAdd('', 0)" />
          <div v-else class="gd-function-task-wrapper">
            <gd-function-task v-for="({ task, key }, i) in conditionTrueTask" :key="key" :task="task" :sensor="sensor"
              :relay="relay" :function="props.function" :style="`z-index: ${conditionTrueTask.length - i};`"
              @remove="conditionTrueTaskRemove(i)" @add-prev="conditionTrueTaskAdd('', i)"
              @add-next="conditionTrueTaskAdd('', i + 1)" @update="(task) => conditionTrueTaskUpdate(task, i)" />
          </div>
        </div>
        <div class="gd-function-task-section">
          <span class="gd-function-task-section-name gd-headline-6">
          ...
          <gd-input-button v-if="conditionFalseTask.length === 0" label="Add task" type="background"
            @clicked="conditionFalseTaskAdd('', 0)" />
          <div v-else class="gd-function-task-wrapper">
            <gd-function-task v-for="({ task, key }, i) in conditionFalseTask" :key="key" :task="task" :sensor="sensor"
              :relay="relay" :function="props.function" :style="`z-index: ${conditionFalseTask.length - i};`"
              @remove="conditionFalseTaskRemove(i)" @add-prev="conditionFalseTaskAdd('', i)"
              @add-next="conditionFalseTaskAdd('', i + 1)" @update="(task) => conditionFalseTaskUpdate(task, i)" />
          </div>
        </div>
      </div>
      ...
      <div v-else-if="kind === 'command'" class="gd-function-task-body">
        ...
        <div class="gd-function-task-input-wrapper" style="margin-top: 0.5rem">
          <gd-input-select :input="taskDeviceValueInput" style="width: calc((100% - 0.5rem) / 2)" />
          <gd-input-select :input="taskDeviceLockInput" style="width: calc((100% - 0.5rem) / 2)" />
        </div>
      </div>
    </div>
    <div class="gd-function-task-add">
...
const command = computed<FunctionTaskCommand>(() => {
  ...
  if (relay) {
    ...
    if (typeof value === "boolean") {
      command.relay = [relay.id, value, typeof lock === "boolean" ? lock : undefined];
      return command;
    }
  }
...
onMounted(() => {
  ...
  if (props.task.command) {
    ...
    if (props.task.command.relay) {
      const command = props.task.command.relay;
      const relay = props.relay.find((a) => a.id === command[0]);
      if (relay) {
        ...
        taskDeviceValueInput.value.model = {
          name: command[1] ? "On" : "Off",
          value: command[1],
        };
        taskDeviceLockInput.value.model = typeof command[2] === "boolean" ? {
          name: command[2] ? "Locked" : "Unlocked",
          value: command[2],
        } : {
          name: "Unset",
          value: "",
      ...
      const command = props.task.command.function;
      const fn = props.function.find((a) => a.id === command);
      if (fn) {
        taskDeviceInput.value.model = {
          name: fn.name,
          value: fn.id,
