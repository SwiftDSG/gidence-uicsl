<template>
  <div class="gd-menu-input-container">
    ...
    <div class="gd-menu-input-wrapper">
      <gd-input-select class="gd-menu-input" :input="methodInput" />
    </div>
    <div class="gd-menu-input-wrapper">
    ...
    <div class="gd-menu-input-test-container">
      <span class="gd-menu-input-test-label gd-caption-text">Sensor reading test</span>
      <div class="gd-menu-input-test">
        <span class="gd-menu-input-test-value gd-headline-3"
          :class="testError ? 'gd-menu-input-test-value-error' : ''">{{ testValue }}</span>
        <gd-input-button-small :disabled="!sensor" :loading="testLoading" @clicked="test" icon="rotate" type="primary"
...
const methodInput = ref<InputSelectOption<Sensor["method"] | "">>({
  name: "sensor-format",
  label: "Reading data type",
  placeholder: "Select one",
  strict: true,
  ...
  options: [
    {
      ...
      value: "read_holding_register",
    },
    {
      name: "Read Input Register",
      value: "read_input_register",
    }
  ],
});
...
const sensor = computed<Sensor | null>(() => {
  ...
  const method = methodInput.value.model.value;
...
const sensorMethods = {
  read_input_register: "Read Input Register",
...
onMounted(() => {
  if (props.sensor) {
    ...
    methodInput.value.model = {
      name: sensorMethods[props.sensor.method],
      value: props.sensor.method.toString(),
    };
