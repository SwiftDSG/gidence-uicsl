<template>
  <div
    ...
    } ${input.alignment ? `gd-input-${input.alignment}` : ''} ${
      input.disabled ? 'gd-input-disabled' : ''
    }`"
  >
    ...
    <div class="gd-input-container">
      <div v-if="input.icon || input.prefix" class="gd-input-icon-container">
        <gd-svg v-if="input.icon" :name="input.icon" />
        <span
          v-else-if="input.prefix"
          class="gd-input-icon-prefix gd-headline-5"
          >{{ input.prefix }}</span
        >
      </div>
      <input
