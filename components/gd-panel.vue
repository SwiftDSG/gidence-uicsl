<template>
  <div class="gd-component" ref="gdComponent">
    <div class="gd-background" ref="rdBackground"></div>
...
<script lang="ts" setup>
  ...
  const animate = {
    init(rdBackground: HTMLElement, gdPanel: HTMLElement): void {
  ...
  function exit(): void {
    if (rdBackground.value && gdPanel.value) {
      animate.exit(view.value, rdBackground.value, gdPanel.value, () => {
        emits("exit");
      });
    }
  }
  ...
  onMounted(() => {
    ...
    if (rdBackground.value && gdPanel.value) {
      animate.init(rdBackground.value, gdPanel.value);
    }
  });
</script>
