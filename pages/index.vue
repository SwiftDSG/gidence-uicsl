      ...
      <div class="gd-controller" v-if="device">
        <div class="gd-controller-header">
          ...
          <div class="gd-controller-status-container">
            <div class="gd-controller-status" :class="online
            ...
            <gd-input-button-small v-if="view !== 'desktop'" icon="dots" :radius="1" @clicked="
              openMenu({
                controller: {
        ...
        <div v-if="pinnedFunction.length > 0" class="gd-controller-functions"
          ...
          <gd-function v-for="fn in pinnedFunction" :key="fn.id" :function="fn" :pinned="true" @open="
            (fn) =>
              openMenu({
                functionInformation: {
                  function: fn,
        ...
        <gd-ports :controller="device.controller" :ports="ports" :devices="devices || []" :selected="instrumentSelected"
          :style="locked ? 'pointer-events: none' : ''" />
      </div>
    </client-only>
    <gd-menus @shake="emits('shake')" />
    <gd-progress-bar type="fixed" :state="loading ? 'show' : 'hide'" :immediate="true" />
  </div>
</template>
